...
**Always-on-top**:
A **host window** stacking preference: Aoide stays above other apps. Available where the platform can honour it — Windows, macOS, and X11, and Plasma Wayland. Absent from the **options cog** where it cannot (Wayland without KWin: GNOME/Mutter, sway), rather than present and inert.
_Avoid_: keep-above (compositor jargon in product talk); treating a missing row as broken; always-on-top as a per-panel or per-group control
