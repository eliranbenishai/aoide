...
export XDG_DATA_HOME="${XDG_DATA_HOME:-/tmp/aoide-bench-xdg}"
...
run() {
  ...
  timeout 300 "$ROOT/build/aoide" --bench-drag "$panel" --bench-moves "$MOVES" \
...
timeout 300 "$ROOT/build/aoide" --bench-resize --bench-moves "$MOVES" \
