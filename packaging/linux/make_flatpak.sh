...
# com.proximamagnifica.aoide.yml names this directory as its source.
BUNDLE="$ROOT/build/linux/flatpak-bundle"
STATE="$ROOT/build/linux/flatpak-builder"
OUT="$ROOT/build/linux/Aoide-${version}-linux-x86_64.flatpak"
...
AOIDE_BUNDLE_DIR="$BUNDLE" "$ROOT/packaging/linux/stage_bundle.sh" --no-qt
...
flatpak-builder --force-clean --default-branch="$BRANCH" \
  ...
  "$ROOT/packaging/flatpak/com.proximamagnifica.aoide.yml"
flatpak build-bundle "$STATE/repo" "$OUT" com.proximamagnifica.aoide "$BRANCH"
echo "Wrote $OUT"
