...
- **Six panels** — Main Player, Equalizer, Playlist Manager, settings, about and skins share one session. macOS, Windows and Linux/X11 use panel-sized frameless windows. Linux/Wayland contains the panels in a regular resizable window. There is no desktop-sized transparent background.
- **Move / snap ownership** — main's title bar moves the cluster; on Wayland the desktop moves its containing window. Other title bars move only that panel. EQ/playlist snap to main or each other, inside the container on Wayland. Settings, about and skins never snap.
- **App chrome** — panels retain their custom title bars, controls and skins. Wayland additionally uses a desktop-managed frame around the container. Main's title bar shows logo + AOIDE wordmark; the other title bars show their role.
- **No version in the title bar** — the mockup's `AOIDE<sup>1.0</sup>` superscript is dropped (approved delta). The version belongs to the About panel's readout, where it is real and comes from [`VERSION`](../VERSION).
...
- **Options cog** on main (gutter left of the display well), with **Skins** and **Track info** stacked under it. The cog opens Always on top / Open files… / Settings… / About Aoide / Quit. Replaces mockup clutter **O / A / I** (approved delta). Skins are their own freestanding panel — “look pack” is a retired term ([`CONTEXT.md`](../CONTEXT.md): **Skin**). Settings tabs are **General | Audio** (output device and exclusive output).
...
- Bundle **full libmpv** (+ required FFmpeg) on Windows, Linux, and macOS — features first; binary size later. macOS uses the pinned audio-full xcframework (`tool/fetch_full_libmpv.sh`); that DMG ships in **1.1**. Pull-request CI builds, tests and smoke-starts the bundle on every run; release CI uploads a notarized DMG, and one has been installed on a MacBook and played audio.
...
- **Media keys** — play/pause, stop, next and previous, while Aoide is the focused application. They are application shortcuts, not a system-wide registration: there is no MPRIS or SMTC surface, so a media key pressed while another app has focus does nothing for Aoide.
...
- **Empty states** (no first-run flag, no tour): an empty track well paints `THIS LIST IS EMPTY` / `Drop files here, or open one from PLAYLISTS.`; an empty collection well paints `NO SAVED PLAYLISTS` / `Aoide only saves references to your playlist files. You can name them whatever you want, without affecting the files. How you manage the files themselves is up to you.`; the main title is `Drop files to play` while it would otherwise be `No track` and the current list has no rows. The footer drop hint is chrome, not a third well line.
