...
|------|----------------|--------|
| Multi-window host | Three frameless OS windows via `desktop_multi_window` + forked `window_manager`. `SessionHostApp` (main engine) owns settings/docking/`SessionBus` + `PlaybackController`/`PlaylistController`/`EqualizerController`; mounts `MainPlayerWindow`. `SessionClientApp` mounts `EqualizerWindow` on the EQ role (playlist still placeholder until Task 9). EQ session commands (`EqGain` / preamp / enabled / auto / preset) drive the host controller + `EqSnapshotEvent` fan-out; shade via `SetShadedCommand`. Main close quits; EQ/PL close hides. Main minimize hides the visible group (EQ/PL via `WindowController.hide`, main via OS minimize) and restores those windows on main restore (`MinimizeGroupCycle`). Frames applied from `DockingCoordinator.frameFor` | Partial (main + EQ chrome mounted; playlist client pending) |
...
| App chrome / UI | Code-constructed primitives in `lib/ui/chrome/mockup/` plus `MockupMainPlayer` / `MainPlayerWindow` and `MockupEqualizer` / `EqualizerWindow` (`lib/ui/main_player/`, `lib/ui/equalizer/`, `lib/ui/windows/`) — absolute mockup geometry, clutterbar **O / A / I** only, live EQ curve painter ([ADR 0007](adr/0007-code-constructed-mockup-chrome.md)). Playlist panel body still pending (Task 9). Prior `TrampShell` + PNG skin retired for new work | Partial (main + EQ mounted; playlist pending) |
...
| Equalizer | Mockup EQ chrome (`EqualizerWindow`) drives host `EqualizerController` over the session bus; controller calls `EqualizerSink.apply` (still `NoopEqualizerSink` until Task 11). Chrome: On / Auto / Presets, live curve from gains, preamp + ten bands ±12 dB, windowshade. Auto keeps existing Tramp semantics unless the product spec says otherwise | Partial (UI + state + sink seam); audible EQ gated |
...
- **Multi-window chrome** — OS windows + session bus are wired; main + EQ mount mockup chrome (`MainPlayerWindow`, `EqualizerWindow`); playlist still placeholder until Task 9 ([ADR 0006](adr/0006-multi-window-docking.md)).
