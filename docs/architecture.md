...
| Zoom | Global discrete zoom (six steps 100–300%, persisted, display-fit gating). Main title-bar −/+ (and shortcuts) resize all three HWNDs (`logical × zoom`) and scale each panel via root `ZoomedCanvas` (`FittedBox` + `ZoomScope`) per [ADR 0002](adr/0002-fixed-canvas-zoom.md). Playlist user size stored in logical coordinates | Implemented |
