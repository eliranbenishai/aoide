...
| Theme / tokens | `MockupTokens` + `TrampMetrics` (825×348 / 825×348 / 825×696, titleBar 42) mirror mockup `:root` / classic×3 grid. Chrome reads palette/materials/fonts via `LookScope` → `ResolvedLook` (`TrampColors` / `TrampText`). **Look packs:** `lib/look/` parse/merge/catalog/install + `LookController` (host); session bus syncs `LookSnapshotEvent` to EQ/PL engines ([design](superpowers/specs/2026-08-09-look-pack-format-design.md)) | Implemented |
