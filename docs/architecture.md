...
| Theme / tokens | `MockupTokens` + `TrampMetrics` (825×348 / 825×348 / 1073×696 playlist / 520×420 settings / 480×360 about, titleBar 42; playlist collection panel 240 default / 180 min / 8 divider, and the derived `playlistMinWithCollection` 828×280) mirror mockup `:root` / classic×3 grid. **Qt host:** `qt/src/look.*` parses `skin.json` / legacy `look.json`, merges `extends`, and builds `ChromeTokens` via the same LookPaint lifts as Dart. `SkinController` catalogs `<support>/skins` plus bundled `skins/` (`TRAMP_SKINS_DIR`), seeds missing packs into the support dir, and Settings → Skins activates a pack for all five windows. Missing pack fonts keep TrampCondensed / TrampMono. Dart `LookScope` / `lib/look/` remains the frozen reference ([design](superpowers/specs/2026-08-09-look-pack-format-design.md)) | Implemented (Qt + Dart) |
| Zoom | Global discrete zoom (eight steps 50–300%, default 75%, persisted). Main title-bar −/+ resizes all five windows (`logical × zoom`); the current percent sits left of that cluster. Chrome rasterizes at widget size × device pixel ratio so glyphs stay sharp at every step. Playlist freely resizes. Dart `ZoomedCanvas` is the frozen reference ([ADR 0002](adr/0002-fixed-canvas-zoom.md)) | Implemented (Qt) |
...
| Equalizer | Mockup EQ chrome drives `EqualizerSettings` on the host; `buildEqualizerAf` → mpv `af` lavfi equalizer. Chrome: On / Auto / Presets, live curve, preamp + ten bands ±12 dB. Band/preamp drags update **EQ only** (cached chassis + live curve/thumbs), debounce the lavfi graph, and must not rebuild main/playlist chrome. Dart `EqualizerController` is the frozen reference | Implemented (UI + audible sink) |
| Spectrum | PCM decode via a throwaway mpv `ao=pcm` instance, STFT folded to **20** log bands, frames indexed by the playback clock (polled on the 33 ms ticker — mpv `time-pos` is not observed). Honest silence until the spectrogram is ready. Live ticks composite cheap bars/clock/seek over a **cached main chassis** — they must not rerun Gaussian-blur chrome or repaint EQ/PL/settings/about. `--dump-chrome` still paints the golden demo bars. Dart `SpectrumAnalyzer` is the frozen reference | Implemented (Qt) |
