...
| Platform | Frameless multi-window APIs (**windows-x64** on Windows); file open / DnD / launch args; file associations; OS media controls; settings persistence (`TrampSettings`: zoom, AOT, mono, five window frames, dock edges, EQ curve, general prefs, skins, Playlist Manager collection width / collapsed — migrates legacy `lowerRegion` / `activeLookId` / `looksDirectory`); `session_resume.json` for playback index/position when resume-last-session is on; `playlists.json` + `playlist_tracks.json` for the playlist collection and `altered_playlist.json` for an altered current playlist (content, so Reset Settings spares them like installed skins); secondary windows skip the Windows taskbar | Partial (settings + session host wired; file associations / second-instance still hardening) |
