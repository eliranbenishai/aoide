...
|------|----------------|--------|
| Multi-window host | Three frameless OS windows via `desktop_multi_window` + forked `window_manager`. `SessionHostApp` (main engine) owns settings/docking/`SessionBus` + `PlaybackController`/`PlaylistController`/`EqualizerController`; mounts `MainPlayerWindow`. `SessionClientApp` mounts `EqualizerWindow` / `PlaylistWindow` on secondary roles. EQ + playlist session commands drive host controllers; `EqSnapshotEvent` / `PlaylistSnapshotEvent` / `PlaybackSnapshotEvent` fan-out; shade via `SetShadedCommand`; playlist edge resize reports `ResizePlaylistCommand` → `DockingCoordinator.resizePlaylist`. Title-bar drag uses OS `startDragging` for the active window; `onWindowMove` → `DockingCoordinator.move` (snap deferred) → coalesced **sibling-only** position applies; full frame + snap + persist + `DockSnapshotEvent` on `onWindowMoved`. Main close quits; EQ/PL close hides. Main minimize hides visible secondaries (`WindowController.hide`) then OS-minimizes main; restore shows them again (`MinimizeGroupCycle`). **Windows taskbar:** secondaries use `skipTaskbar: true` (style fallback if needed) so only main appears. Frames from `DockingCoordinator.frameFor` | Implemented (taskbar hide: polish pass) |
| Session bus | `lib/ui/session/`: JSON `SessionEvent` / `SessionCommand` codec; host registers unidirectional `WindowMethodChannel('tramp/session')`; clients send commands (including `MoveWindowCommand`); host pushes events/frames via per-window `WindowController.invokeMethod` (`apply_frame` supports `positionOnly` during drag) | Implemented |
| Docking | `DockingCoordinator` + `DockLayout` + `DockDragArea` + `DockMoveCoalescer` (`lib/ui/docking/`): main title drag translates all **visible** satellites (edges irrelevant); EQ/PL peel on drag and snap only on finalize (EQ any side; PL top/bottom + optional L/R flush within 20px); undock via peel / Shift / separation; hide clears edges; shade height = `TrampMetrics.titleBar`; OS `startDragging` ([ADR 0006](adr/0006-multi-window-docking.md), [polish design](superpowers/specs/2026-08-09-ui-polish-docking-taskbar-design.md)) | Implemented (rules: polish pass) |
| App chrome / UI | Code-constructed primitives in `lib/ui/chrome/mockup/` plus `MockupMainPlayer` / `MainPlayerWindow`, `MockupEqualizer` / `EqualizerWindow`, and `MockupPlaylist` / `PlaylistWindow` — absolute mockup geometry, clutterbar **O / A / I** only, live EQ curve, EQ band bottom→thumb spectrum-gradient fill, compact EQ/PL titles (role only), `.wbtn` inset bevel chrome, resizable playlist list/footer ([ADR 0007](adr/0007-code-constructed-mockup-chrome.md)). PNG graphite / `TrampShell` deleted | Implemented (polish / goldens hardening) |
...
| Platform | Frameless multi-window APIs (**windows-x64** on Windows); file open / DnD / launch args; file associations; OS media controls; settings persistence (`TrampSettings`: zoom, always-on-top, mono, per-window frames, dock edges, EQ curve — migrates legacy `lowerRegion`); secondary windows skip the Windows taskbar | Partial (settings + session host wired; taskbar skip: polish pass; file associations / second-instance still hardening) |
...
- **Mockup / polish goldens** — side-by-side mockup fidelity / platform golden sets still hardening; docking drag is wired; 2026-08-09 polish (button bevel, dock rules, EQ fill, compact titles, taskbar) tracked in [`2026-08-09-ui-polish-docking-taskbar-design.md`](superpowers/specs/2026-08-09-ui-polish-docking-taskbar-design.md).
