...
**Quit.** Main close writes resume + spins, waits for the background workers to notice they have been cancelled (tens of milliseconds), then exits. Persist during the session (debounced), not on teardown. Altered current playlist is kept continuously.
