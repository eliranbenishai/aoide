...
|------|----------------|--------|
| Multi-window host | Three frameless OS windows via `desktop_multi_window` + forked `window_manager`. `SessionHostApp` (main engine) owns settings/docking/`SessionBus` + `PlaybackController`/`PlaylistController`/`EqualizerController`; mounts `MainPlayerWindow`. `SessionClientApp` mounts `EqualizerWindow` / `PlaylistWindow` on secondary roles. EQ + playlist session commands drive host controllers; `EqSnapshotEvent` / `PlaylistSnapshotEvent` / `PlaybackSnapshotEvent` fan-out; shade via `SetShadedCommand`; playlist edge resize reports `ResizePlaylistCommand` → `DockingCoordinator.resizePlaylist`. Title-bar drag uses `DockDragArea` → `MoveWindowCommand` (secondaries) or host-local move (main) → `DockingCoordinator.move` → apply frames for the docked group; persist + `DockSnapshotEvent` on pan-end. Main close quits; EQ/PL close hides. Main minimize hides the visible group (EQ/PL via `WindowController.hide`, main via OS minimize) and restores those windows on main restore (`MinimizeGroupCycle`). Frames applied from `DockingCoordinator.frameFor` | Implemented |
| Session bus | `lib/ui/session/`: JSON `SessionEvent` / `SessionCommand` codec; host registers unidirectional `WindowMethodChannel('tramp/session')`; clients send commands (including `MoveWindowCommand`); host pushes events/frames via per-window `WindowController.invokeMethod` | Implemented |
| Docking | `DockingCoordinator` + `DockLayout` + `DockDragArea` (`lib/ui/docking/`): edge snap (12px), sticky group drag, undock via >48px break or Shift; shade height = `TrampMetrics.titleBar`; host applies pixel frames to OS windows ([ADR 0006](adr/0006-multi-window-docking.md)) | Implemented |
| App chrome / UI | Code-constructed primitives in `lib/ui/chrome/mockup/` plus `MockupMainPlayer` / `MainPlayerWindow`, `MockupEqualizer` / `EqualizerWindow`, and `MockupPlaylist` / `PlaylistWindow` (`lib/ui/main_player/`, `lib/ui/equalizer/`, `lib/ui/playlist/`, `lib/ui/windows/`) — absolute mockup geometry, clutterbar **O / A / I** only, live EQ curve, resizable playlist list/footer ([ADR 0007](adr/0007-code-constructed-mockup-chrome.md)). PNG graphite / `TrampShell` deleted | Implemented (mockup fidelity / goldens still hardening) |
| Theme / tokens | `MockupTokens` + `TrampMetrics` (825×348 / 825×348 / 825×696, titleBar 42) mirror mockup `:root` / classic×3 grid; `TrampColors` / `TrampText` facade cyan phosphor + TrampCondensed/TrampMono | Implemented (mockup chrome path) |
| Zoom | Global discrete zoom (six steps 100–300%, persisted, display-fit gating). Main title-bar −/+ (and shortcuts) scale all three windows; pixel size = logical × zoom. Playlist user size stored in logical coordinates ([ADR 0002](adr/0002-fixed-canvas-zoom.md)) | Implemented |
...
| Spectrum | `SpectrumAnalyzer` (`lib/analysis/`): second mpv `ao=pcm` decode → isolate STFT → 20 bars; engine publishes `synthetic: false` in normal play | Implemented |
| Mono | `MonoController` / `PlayerEngine.setForceMono` → mpv `audio-channels` `mono`/`auto`; main Mono button → settings + engine | Implemented |
| Platform | Frameless multi-window APIs; file open / DnD / launch args; file associations; OS media controls; settings persistence (`TrampSettings`: zoom, always-on-top, mono, per-window frames, dock edges, EQ curve — migrates legacy `lowerRegion`) | Partial (settings + session host wired; file associations / second-instance still hardening) |
...
- **Mockup goldens** — side-by-side mockup fidelity / platform golden sets still hardening; docking drag is wired end-to-end ([ADR 0006](adr/0006-multi-window-docking.md)).
