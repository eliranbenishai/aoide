...
| Platform | Frameless multi-window APIs; file open / DnD / launch args; file associations; OS media controls; settings persistence (`TrampSettings`: zoom, always-on-top, mono, per-window frames, dock edges, EQ curve — migrates legacy `lowerRegion`) | Partial (settings + session host; docking drag UX pending) |
...
- **Multi-window chrome** — OS windows + session bus are wired; main / EQ / playlist mount mockup chrome. Docking drag UX and full side-by-side mockup goldens still hardening ([ADR 0006](adr/0006-multi-window-docking.md)).
