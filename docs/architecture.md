...
| Libmpv bundle | `third_party/libmpv` pins + fetch; Windows DLL + import library / Linux staged `.so` / macOS audio-full `Mpv.xcframework` (universal). The macOS fetch refuses a slim `--disable-filters` build. |
