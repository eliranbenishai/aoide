...
| App chrome / UI | Code-constructed primitives in `lib/ui/chrome/mockup/` plus `MockupMainPlayer` / `MainPlayerWindow` (`lib/ui/main_player/`, `lib/ui/windows/`) — absolute mockup geometry, clutterbar **O / A / I** only ([ADR 0007](adr/0007-code-constructed-mockup-chrome.md)). EQ/PL panel bodies still pending (Tasks 8–9). Prior `TrampShell` + PNG skin retired for new work | Partial (main window mounted; EQ/PL pending) |
...
- **Multi-window chrome** — OS windows + session bus are wired; main player mounts mockup chrome (`MainPlayerWindow`); EQ/PL still placeholders until Tasks 8–9 ([ADR 0006](adr/0006-multi-window-docking.md)).
