...
| Docking | `DockingCoordinator`: edge/corner snap, sticky group drag, undock via break threshold and/or modifier; layout uses windowshade height when collapsed ([ADR 0006](adr/0006-multi-window-docking.md)) | Target (redesign) |
...
| Theme / tokens | Checked-in token + geometry map mirroring mockup `:root` / CSS; Flutter constants mirror CSS — do not invent a parallel theme. Cyan phosphor (not chartreuse) | Target (redesign) |
...
| Playlist | Open/save M3U/M3U8, add/remove/reorder/sort, play from selection, restore last playlist (`PlaylistController`, `M3uCodec`, `PlaylistStore`) | Implemented (logic); UI moves to detachable window |
