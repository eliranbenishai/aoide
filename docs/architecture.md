...
| App chrome / UI | Code-constructed primitives in `lib/ui/chrome/mockup/` (shell, title bar, screen, button, slider, LED, icons, plate/rail/rivet) from mockup tokens + geometry — no PNG panel faces ([ADR 0007](adr/0007-code-constructed-mockup-chrome.md)). Main/EQ/PL panel bodies still pending (Tasks 7–9). Main clutterbar **O / A / I** only | Partial (primitives + goldens; panels not mounted yet). Prior `TrampShell` + PNG skin retired for new work |
...
- **Multi-window chrome** — OS windows + session bus are wired; mockup chrome primitives exist under `lib/ui/chrome/mockup/`; EQ/PL still show placeholders and main is a host shell until Tasks 7–9 mount full panels ([ADR 0006](adr/0006-multi-window-docking.md)).
