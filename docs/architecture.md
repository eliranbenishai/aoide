...
| Zoom | Global discrete zoom (eight steps 50–300%, default 75%, persisted). Main title-bar −/+ resizes all five windows (`logical × zoom`); the current percent sits between − and +. Chrome rasterizes at widget size × device pixel ratio so glyphs stay sharp at every step. Playlist freely resizes. Dart `ZoomedCanvas` is the frozen reference ([ADR 0002](adr/0002-fixed-canvas-zoom.md)) | Implemented (Qt) |
