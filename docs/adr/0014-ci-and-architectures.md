...
Apple notarization secrets and any Store identity variables live in GitHub, not on a laptop. Workflows: `.github/workflows/ci.yml` (PR/main tests), `open-pr.yml` / `merge-if-green.yml` (PR loop, [ADR 0018](0018-pr-ci-auto-merge.md)), and `release.yml` (tag `v*` packages). How to cut a release and which secrets to set: [`distribution.md`](../distribution.md). Windows ARM and Linux ARM AppImage are explicit non-goals until a real need. The website EXE remains unsigned; SmartScreen is documented, not “fixed” by CI. The macOS DMG job waits on the Qt Mac host ([ADR 0016](0016-qt-for-v1.md)).

Feature branches: push opens a PR; Qt CI is the GitHub review; squash-merge happens only when that CI is green ([ADR 0018](0018-pr-ci-auto-merge.md)).
