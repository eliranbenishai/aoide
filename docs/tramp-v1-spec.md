...
- **Move / snap ownership** — dragging the main title bar translates the host (cluster moves as a unit; host size unchanged). Dragging EQ, playlist, settings, or about moves only that panel; siblings stay put; the host is the tight union of visible panels. Snap only from EQ/playlist: EQ any side of any panel; playlist **top/bottom only**, with left/right flush when already within threshold. Details: [`2026-08-09-ui-polish-docking-taskbar-design.md`](superpowers/specs/2026-08-09-ui-polish-docking-taskbar-design.md).
